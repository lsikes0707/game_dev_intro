...
-- our Paddle class, which stores position and dimenensions for each Paddle
...
function love.load()
    ...
    -- set up our sounds effects; later, we can just index this table and
...
function love.update(dt)
    ...
    elseif love.keyboard.isDown('s') then
        player1.dy = PADDLE_SPEED   -- go down
    else
        player1.dy = 0
    end
    ...
    if love.keyboard.isDown('up') then
        player2.dy = -PADDLE_SPEED    -- go up
    elseif love.keyboard.isDown('down') then
        player2.dy = PADDLE_SPEED    -- go down
    else
        player2.dy = 0
    end
