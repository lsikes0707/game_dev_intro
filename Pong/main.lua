...
function love.update(dt)
    ...
    if love.keyboard.isDown('w') then
        ...
        player1Y = math.max(0, player1Y + -PADDLE_SPEED * dt)    -- go up
    elseif love.keyboard.isDown('s') then
        -- add positive paddle speed to current Y scaled by deltaTime
        -- math.min returns the lesser of two values; bottom of the edge minus
        -- paddle height and player Y will ensure we don't go below it
        player1Y = math.min(VIRTUAL_HEIGHT - 20, player1Y + PADDLE_SPEED * dt)     -- go down
    end

    -- player 2 movement
    if love.keyboard.isDown('up') then
        -- add negative paddle speed to current Y scaled by deltaTime
        player2Y = math.max(0, player2Y + -PADDLE_SPEED * dt)    -- go up
    elseif love.keyboard.isDown('down') then
        -- add positive paddle speed to current Y scaled by deltaTime
        player2Y = math.min(VIRTUAL_HEIGHT - 20, player2Y  + PADDLE_SPEED * dt)    -- go down
    end
...
function love.keypressed(key)
  ...
  elseif key == 'enter' or key == 'return' then
    if gameState == 'start' then
        ...
        -- the and/or pattern here is Lua's way of accomplishing a
        -- ternary operation in other programming languages like C
        ballDX = math.random(2) == 1 and 100 or -100
